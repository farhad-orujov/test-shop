...
export default function PopularPage() {
  ...
  useEffect(() => {
    const fetchPopular = async () => {
      try {
        const res = await fetch('/api/products?tag=popular');
        if (res.ok) {
          const data = await res.json();
          setProducts(data || []);
        } else {
          console.error('Failed to fetch popular items', await res.text());
        }
      } catch (err) {
    ...
    <main className="container mx-auto px-4 py-8">
      ...
      <section className="relative">
        ...
        <div
          ...
          className={clsx('flex gap-4 overflow-x-auto scroll-smooth py-4 px-8', 'snap-x snap-mandatory')}
          style={{ scrollPadding: '1rem' }}
        >
          {loading && <div className="w-full flex items-center justify-center py-12">Loading...</div>}

          {!loading && products.length === 0 && (
            <div className="w-full text-center text-zinc-500 py-12">No popular items found.</div>
          )}
          ...
          {products.map((p) => (
            <div key={p._id} className="snap-start flex-shrink-0 w-[320px] sm:w-[360px] md:w-[420px]">
              <GoodsCard
                ...
                tags={p.tags}
                colorVariants={p.colorVariants?.map((v) => ({ colorName: v.color || '', colorCode: v.colorCode || '', image: v.image || '' }))}
              />
            </div>
          ))}
        </div>
